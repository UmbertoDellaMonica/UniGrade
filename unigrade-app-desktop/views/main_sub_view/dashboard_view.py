...
class DashboardView(ctk.CTkFrame):
    def __init__(self, master, student_id):
        ...
        if not self.student:
            self.student = {"nome":"N/D", "cognome":"N/D", "matricola":"N/D", "corso":"N/D"}
    ...
    def init_avatar(self):
        avatar_frame = ctk.CTkFrame(self.scrollable_frame, corner_radius=20, fg_color="#2a2a2a")
        ...
        self.avatar_label = ctk.CTkLabel(avatar_frame, text="Avatar", font=("Arial", 18, "bold"))
        self.avatar_label.pack(pady=(20,10))

        self.avatar_img_label = ctk.CTkLabel(avatar_frame, text="(clicca per caricare foto)", width=150, height=150, fg_color="#444", corner_radius=75)
    ...
    def init_student_info(self):
        info_frame = ctk.CTkFrame(self.scrollable_frame, corner_radius=20, fg_color="#2a2a2a", border_width=2, border_color="#4da6ff")
        ...
        ctk.CTkLabel(info_frame, text=nome_completo, font=("Arial", 24, "bold")).pack(pady=(20,5))
        ctk.CTkLabel(info_frame, text=f"{self.student['corso']}", font=("Arial", 16, "italic"), text_color="#4da6ff").pack(pady=(0,15))
        ...
        ctk.CTkLabel(matricola_frame, text=f"🆔 Matricola: {self.student['matricola']}", font=("Arial", 14, "bold")).pack(padx=10, pady=5)
        ...
        ctk.CTkLabel(summary_frame, text=f"📚 Esami completati: {len(exams)}", font=("Arial", 14)).pack(pady=5)
        ctk.CTkLabel(summary_frame, text=f"🎯 Media ponderata: {media:.2f}" if exams else "🎯 Media ponderata: N/A", font=("Arial", 14)).pack(pady=5)
        ctk.CTkLabel(summary_frame, text=f"💎 CFU totali: {totale_cfu}", font=("Arial", 14)).pack(pady=5)

    # ------------------------
    # GRAFICO ANDAMENTO VOTI
    # ------------------------
    def init_exam_chart(self):
        exams_frame = ctk.CTkFrame(self.scrollable_frame, corner_radius=15, fg_color="#2a2a2a")
        exams_frame.grid(row=1, column=0, columnspan=3, sticky="nsew", padx=10, pady=(20,10))
        
        exams = get_exams(self.student_id)
        if not exams:
            ctk.CTkLabel(exams_frame, text="Nessun esame inserito!", font=("Arial", 16), fg_color="#2a2a2a").pack(pady=20)
            return
        ...
        ax.plot(range(1, len(exam_scores)+1), exam_scores, marker="o", color="#e63946", linewidth=2)
        ax.set_ylim(18, 31)
        ...
        ax.set_xticklabels(range(1, len(exam_scores)+1), rotation=45, ha="right", fontsize=10, color="white")
