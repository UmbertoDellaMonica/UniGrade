...
class LoginView:
    def __init__(self, master, previous_view=None):
        ...
        ctk.CTkCheckBox(self.frame, text="Ricordami", variable=self.remember_var).pack(pady=5)
    ...
    def generate_token(self, matricola, passowrd_hashed):
        ...
        payload = {"matricola": matricola, "pw_hash": passowrd_hashed, "exp": exp.timestamp()}
    ...
    def auto_login(self):
        ...
        if student_id:
            show_temp_message(self.frame, "✅ Login automatico effettuato!", color="green")
            self.master.after(1200, lambda: self.open_main(student_id))
        else:
            clear_token()
            show_temp_message(self.frame, "⚠️ Token scaduto o invalido, inserisci le credenziali!", color="red")
    ...
    def do_login(self):
        ...
        if student_id:
            ...
            if self.remember_var.get():
                ...
                token = self.generate_token(matricola, passowrd_hashed=hash_password(password))
                # Save token session 
                save_token(token)
            else:
                # Delete token 
